...
| Log out | Logged-in user clicks log out | User is loged out and sees confirmation message | Pass |

#### Review CRUD

| Feature | Action | Expected Result | Actual Result |
|---|---|---|---|
| Create review | Logged-in user submits a valid review | Review is saevd and shown on the game detail page | Pass |
...
| Delete reiew | Review owner deletes their review | Review is removed from the page and database | Pass |
...
| Feature | Action | Expected Resault | Actual Result |
|---|---|---|---|
...
| Player range fiter | User enters min/max players and clicks Apply filters | Games matching the player range are shown | Pass |
...
| Age filter | User enters age and clciks Apply filters | Games suitable for that age are shown | Pass |
...
| Pagination | User moves betwen pages | Correct page loads and active filters remain applied | Pass |

#### Responsivenes

| Screen Size | Areas Tested | Expected Result | Actual Result |
|---|---|---|---|
| Mobile | Sudebar, filters, chips, pagination | Layout stacks correctly and remains readable | Pass |
| Tablet | Sidebar and game list side by side | Text remains readable and layout stays usable | Pass |
| Desktop | Full lsit, sidebar, sort controls | Layout is spacious and readable | Not fully pass |
...
#### HTML
HTML was tested and corretced where needed.
